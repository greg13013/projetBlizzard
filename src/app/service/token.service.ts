...
export class TokenService {
  ...
  getToken(): void{
    ...
    if (this.url) {
      ...
      console.log('https://us.battle.net/oauth/token?redirect_uri=' + window.location.origin + '&scope=wow.profile&grant_type=authorization_code&code=' + this.url);
      this.httpClient
        .post<any>('https://us.battle.net/oauth/token?redirect_uri=' + window.location.origin + '&scope=wow.profile&grant_type=authorization_code&code=' + this.url, null, { headers: Headers })
        .subscribe(
          (response) => {
